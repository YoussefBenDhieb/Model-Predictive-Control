    ...
    if (sdata.size() > 2 && sdata[0] == '4' && sdata[1] == '2') {
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          //predict the new state after latency
          double px0 =0;
          double py0=0;
          double psi0 = 0;
          const double Lf = 2.67;
          const double latency = 0.1;
          double px1 = px0 + v * cos(psi0) * latency;
          double py1 = py0 + v * sin(psi0) * latency;
          double psi1 = psi0 - v * steering_angle / Lf * latency;
          double v1 = v + a * latency;
          double f = coeffs[0] + coeffs[1] * px + coeffs[2] * pow(0, 2) + coeffs[3] * pow(0, 3);
          double psides = atan(coeffs[1] + 2 * coeffs[2] * px0 + 3 * coeffs[3] * pow(px0, 2));
          double cte1 = f - py0 + v * sin(epsi) * latency;
          double epsi1 = psi0 - psides - v * steering_angle / Lf * latency;
